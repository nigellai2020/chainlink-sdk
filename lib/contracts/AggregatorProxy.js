...
const AggregatorProxy_json_1 = __importDefault(require("./AggregatorProxy.json"));
class AggregatorProxy extends eth_contract_1.Contract {
    ...
    assign() {
        ...
        let getAnswer_call = async (roundId) => {
            let result = await this.call('getAnswer', [this.wallet.utils.toString(roundId)]);
            return new eth_contract_1.BigNumber(result);
        };
        this.getAnswer = getAnswer_call;
        let getRoundData_call = async (roundId) => {
            let result = await this.call('getRoundData', [this.wallet.utils.toString(roundId)]);
            return {
                roundId: new eth_contract_1.BigNumber(result.roundId),
        ...
        let getTimestamp_call = async (roundId) => {
            let result = await this.call('getTimestamp', [this.wallet.utils.toString(roundId)]);
            return new eth_contract_1.BigNumber(result);
        };
        ...
        let phaseAggregators_call = async (param1) => {
            let result = await this.call('phaseAggregators', [this.wallet.utils.toString(param1)]);
            return result;
        };
        ...
        let proposedGetRoundData_call = async (roundId) => {
            let result = await this.call('proposedGetRoundData', [this.wallet.utils.toString(roundId)]);
            return {
                roundId: new eth_contract_1.BigNumber(result.roundId),
                answer: new eth_contract_1.BigNumber(result.answer),
                startedAt: new eth_contract_1.BigNumber(result.startedAt),
